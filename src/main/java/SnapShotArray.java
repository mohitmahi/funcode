...
* int get(index, snap_id) returns the value at the given index, at the time we took the snapshot with the given snap_id
     */
// Option 1: List<TreeMap<>>  List[i] ==> TreeMap<SnapId, Value>  get O(Log(SnapCount)) set O(log(SnapCount) but will O(1) as SnapCount will be largest so far

// Option 2: List<HashMap>  List[i] ==> HashMap<SnapId, Value>    get O(SnapCount)  set O(1) // Good if SnapCount is small

// Option 3: List<HashMap> and a Map<Index,lastSnapId>  to optimize get scan lastSnapId << passedSnapId.
